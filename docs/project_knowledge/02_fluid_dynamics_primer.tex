...
\paragraph{The homogeneous-operator trap.} A sharper, project-specific subtlety: the steady Stokes operator is \emph{linear and homogeneous} (zero body force), so $\mathbf{u}\equiv\mathbf{0},\,p\equiv\text{const}$ is an \emph{exact} solution. The PDE residual and the no-slip BC are both minimised by this trivial field, and a few sparse interior velocity samples are too weak to hold the magnitude against them --- so an under-constrained PINN slides toward $\mathbf{u}\to\mathbf{0}$ and grossly under-predicts WSS. What pins the magnitude is an \emph{open-boundary} datum: a prescribed inflow velocity or volumetric flux $Q$ (in real 4D-flow MRI, simply measured at the inlet) excludes the trivial solution and makes the magnitude well-posed. The PINN implementation enforces exactly this, plus auxiliary dense-prior and magnitude-floor regularisers; see Document 3, \emph{Avoiding collapse to the trivial solution}.
