...
\end{align*}
The PDE residual $\mathbf{R}_{\text{NS}}$ is computed by autograd: the network's output is differentiated with respect to its input.
...
\textit{Project relevance:} hard no-slip constraint via SDF is one of your candidate methodological tweaks.
...
\item \textbf{Convergence to trivial $\mathbf{u}\equiv\mathbf{0}$.} Indicates loss-balance pathology: physics + no-slip dominate, data term too weak. Increase $\lambda_{\text{data}}$ or rescale.
...
\]
then $\boldsymbol{\tau}_w$ from \eqref{eq:WSS} (Document 2). \emph{The network is differentiable everywhere, by construction; this is the central PINN advantage over voxel-based finite differences.}
